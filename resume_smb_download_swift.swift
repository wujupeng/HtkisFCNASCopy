...
func mountedPath(for components: SMBURLComponents, mountEntries: [SMBMountEntry]) -> URL? {
    ...
    for p in components.pathComponents.dropFirst(best.remotePathComponents.count) {
        resolved.appendPathComponent(p, isDirectory: false)
    }
    return resolved
}
...
func resolveSMBMountedPath(from smbURLString: String) throws -> URL {
    ...
    var src = mountBase
    for p in components.remainder { src.appendPathComponent(p, isDirectory: false) }
    return src
}
...
func listFilesRecursively(
    ...
    while let u = e?.nextObject() as? URL {
        ...
        if v?.isRegularFile == true {
            ...
            if !quiet {
                ...
                if now - lastTick >= max(0.1, progressIntervalSeconds) {
                    fputs("Scanning… files=\(count)\n", stderr)
                    lastTick = now
                }
            }
        }
    ...
    if !quiet {
        fputs("Scanning done: files=\(count)\n", stderr)
    }
    return out
}
...
do {
    ...
    for s in a.srcs {
        ...
        if isDirectoryURL(srcRoot) {
            anyDir = true
            let files = try listFilesRecursively(root: srcRoot, quiet: a.quiet, isCancelled: cancellationState.isCancelled)
            for f in files {
