...
- 为避免“共享名大小写不一致导致写到本机同名目录”的问题：
  ...
  - 且要求该路径必须是挂载点（不是普通目录）
- 另外也兼容 macOS 把 `smb://HOST/SHARE/subdir` 直接挂载成 `/Volumes/subdir` 的情况；程序会优先识别最具体的已挂载远端路径，避免上传/下载时落到错误目录

## 退出码与错误信息

- `0`：成功
- `130`：用户取消，已保留 `.part` / `.resume.json`，再次执行可继续
...
swiftc resume_smb_copy_swift.swift -O -o HtkisFCNASCopy.app/Contents/Resources/resume-smb-copy
swiftc resume_smb_download_swift.swift -O -o HtkisFCNASCopy.app/Contents/Resources/resume-smb-download
codesign --force --deep --sign - HtkisFCNASCopy.app
```
...
- 下载文件夹时增加“扫描中”进度提示，避免文件很多时 GUI 看起来无响应

## 1.0.4 优化说明

- 重新对齐桌面应用 bundle 与源码版本，避免 `.app` 内置 CLI 落后于仓库代码
- README 的应用打包流程补齐了内置 `resume-smb-copy` / `resume-smb-download` 的编译与签名步骤
- 后续从 `HtkisFCNASCopy.app` 启动的上传/下载、停止、续传行为会和当前源码保持一致

## 1.0.3 优化说明

- 增加 CLI 对 `SIGINT` / `SIGTERM` 的优雅取消处理
- 取消时会保留 `.part` 与 `.resume.json`，下次运行可继续
- 桌面端点击“停止”后会优先显示“已停止（可再次点击开始继续）”
- 补充针对“取消后保留续传状态”的核心库测试

## 1.0.2 优化说明

- 补充了续传相关的稳定性测试：无元数据续传、源变化拦截与 `--force`、`.part` 异常处理
- 补充了目标已存在时的行为测试：不同大小默认报错、`--overwrite` 可覆盖

## 1.0.1 优化说明

- 优化了 CLI / 核心库的 SMB 挂载点解析逻辑
- 兼容 `share` 与 `share/子目录` 被 macOS 挂载为不同 `/Volumes/...` 名称的场景
- 结合真实大文件上传/下载测试，补充了针对“优先选择最具体挂载点”的自动化测试
